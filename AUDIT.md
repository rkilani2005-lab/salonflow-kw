...
| B.15 | Plan / subscription limit enforcement | ✅ fixed (this commit) — race condition closed: pending invites now count against the limit in both the edge function and the UI, edge function uses get_plan_user_limit RPC for single source of truth instead of hardcoded constants. Client-side PLAN_LIMITS constant still duplicates the mapping as a display hint — flagged for optional RPC-backed refactor. |
