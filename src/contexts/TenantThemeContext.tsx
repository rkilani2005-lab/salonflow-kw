...
// ---------------------------------------------------------------
// Co-brand engine. Reads public.tenant_theme for current tenant,
// writes CSS custom properties into :root so Tailwind / shadcn
// tokens flip to the tenant brand. Falls back to ZAINA defaults.
// ---------------------------------------------------------------
import { createContext, useContext, useEffect, useState, ReactNode } from "react";
import { supabase } from "@/integrations/supabase/client";
import { useTenant } from "./TenantContext"; // your existing tenant hook
...
const ZAINA_DEFAULT: TenantTheme = {
  ...
  logo_dark_url: null,
  favicon_url: null,
  primary_color: "#C0395E",       // rose (existing ZAINA)
  ...
  font_body: "Inter",
  font_arabic: "Tajawal",
  show_powered_by: false,          // ZAINA on its own doesn't show "powered by itself"
  powered_by_text: "",
  powered_by_url: "",
  whatsapp_link: null,
...
const TenantThemeCtx = createContext<Ctx>({
  theme: ZAINA_DEFAULT, loading: true, refresh: () => {},
});
...
export function TenantThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Write CSS vars whenever theme changes
  useEffect(() => {
    ...
    document.title = `${theme.brand_name} — Booking & Inbox`;
  }, [theme]);
...
// Convenience component for the "Powered by ZAINA" stripe
export function PoweredByFooter() {
  ...
  return (
    <a
      ...
      className="fixed bottom-2 right-3 z-40 text-[10px] text-muted-foreground/60
                 hover:text-muted-foreground transition px-2 py-1 rounded
                 bg-background/70 backdrop-blur-sm pointer-events-auto"
    >
