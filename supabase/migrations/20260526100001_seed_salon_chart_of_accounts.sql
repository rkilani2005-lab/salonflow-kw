-- =====================================================================
-- Seed: salon-specific Chart of Accounts + GL mappings (Zaina template)
-- =====================================================================
-- Idempotent — uses ON CONFLICT (tenant_id, code) to skip already-seeded
-- accounts. Safe to re-run on the same tenant; safe to call on multiple
-- tenants without re-creating duplicates. Mapping inserts use upserts on
-- the (tenant_id, mapping_type, source_key) unique constraint.
--
-- Usage (after this migration applies):
--   select public.seed_salon_chart_of_accounts('<tenant_uuid>');
--
-- Two flavours of chart_of_accounts schema exist in the repo (the
-- original enum-typed one, and the Lovable-applied text-typed one). We
-- write to both shapes safely by using account_type::text-castable
-- values that are valid in either.
-- =====================================================================
...
declare
  ...
  -- After insert we resolve account UUIDs by code for the mapping step
...
begin
  ...
  -- =================================================================
  -- 1) Chart of accounts inserts.
  --    Tuple shape: (code, name, name_ar, account_type, account_subtype)
  --    ON CONFLICT (tenant_id, code) means re-running is a no-op.
  -- =================================================================
  ...
  -- =================================================================
  -- 2) Resolve account IDs by code (works whether we just inserted
  --    them or they were already there from a previous seed).
  -- =================================================================
  ...
  select id into v_id_4200 from chart_of_accounts where tenant_id = p_tenant_id and code = '4200';
  select id into v_id_4300 from chart_of_accounts where tenant_id = p_tenant_id and code = '4300';
  select id into v_id_4400 from chart_of_accounts where tenant_id = p_tenant_id and code = '4400';
  select id into v_id_4500 from chart_of_accounts where tenant_id = p_tenant_id and code = '4500';
  ...
  -- =================================================================
  -- 3) GL mappings — driven by service_category enum + product_type +
  --    payment methods. Upsert by (tenant_id, mapping_type, source_key).
  -- =================================================================

  -- ---- REVENUE: SERVICE (credit account is the revenue GL) ----
  -- Debit side comes from the payment method on each POS sale.
  ...
  -- ---- REVENUE: PRODUCT (retail sales) + COGS pairing ----
  -- For each retail sale we'll need TWO journal entries: revenue + COGS.
  -- We model both on the product mapping, debit=COGS, credit=revenue,
  -- with the inventory leg handled by the COGS posting engine.
  ...
  -- ---- COGS / consumables — debit accounts per service category ----
  -- (Posted when a service is performed if linked product usage is tracked.)
  ...
  values
    ...
    -- Operating expenses (debit) typically paid out of bank (credit)
  ...
  -- ---- PAYMENT METHODS (debit account = where the money lands) ----
...
comment on function public.seed_salon_chart_of_accounts(uuid)
  is 'Seed the Zaina Salon-style Chart of Accounts (~60 GLs) and GL mappings (service categories, product types, payment methods, operating expenses) for a tenant. Idempotent — safe to re-run; existing rows are not duplicated and existing mappings are updated to current values.';
