...
-- =====================================================================
-- Provider-agnostic channel schema. Default backend is 'baileys'
-- (self-hosted WhatsApp Web multi-device). Meta Cloud API and other
-- BSPs are supported via the `provider` column — no future migration
-- needed to upgrade a tenant to an official API.
--
-- Tables:
--   conversations    — unified WA/IG/Telegram threads
--   messages         — unified inbound/outbound log
--   channel_accounts — one connected channel per tenant per channel
--   tenant_theme     — co-brand overrides (logo, colors, Powered by)
--   webhook_logs     — raw inbound payloads (debug/replay)
-- =====================================================================

-- ---------- channel_accounts (create first; conversations FKs it) ----
create table if not exists public.channel_accounts (
  ...
  provider_account_id     text,                    -- Baileys session_id == this row's id.text
  display_handle          text,                    -- phone E.164 for WA, @handle for IG
  display_name            text,
...
create table if not exists public.conversations (
  ...
  channel                 text not null check (channel in ('whatsapp','instagram','telegram','linkedin')),
  provider_chat_id        text not null,           -- Baileys JID / Unipile chat_id / etc.
  external_id             text not null,           -- phone / handle
...
comment on table public.conversations    is 'Unified WA/IG/Telegram threads';
comment on table public.messages         is 'Unified inbound/outbound messages across channels';
comment on table public.channel_accounts is 'One row per connected channel per tenant. provider=baileys default.';
comment on table public.tenant_theme     is 'Co-brand overrides (logo, colors, Powered by footer)';
comment on table public.webhook_logs     is 'Raw inbound webhook payloads (debug/replay). TTL ~30d.';
